...
try {
    ...
    $insertStmt = $pdo->prepare("INSERT INTO favourite_albums (userId, album) VALUES (:userId, :album)");

    foreach ($albums as $album) {
        $checkStmt->execute(['userId' => $userId, 'album' => $album]);
        if ($checkStmt->fetchColumn() == 0) {
            $insertStmt->execute(['userId' => $userId, 'album' => $album]);
        }
    }

    echo json_encode(["message" => "Albums added successfully"]);
